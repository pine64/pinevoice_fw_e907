...
static void iperf_client_tcp(void *arg)
{
    ...
    send_buf = (uint8_t *) aos_malloc (IPERF_BUFSZ);
    if (!send_buf) {
        aos_free(arg);
        return;
    }
...
static void iperf_client_tcp_entry(const char *name)
{
    ...
    host = aos_malloc(host_len);//mem will be free in tcpc_entry
    strcpy(host, name);
...
static void iperf_client_udp(void *arg)
{
    ...
    send_buf = (uint8_t *) aos_malloc (IPERF_BUFSZ_UDP);
    if (!send_buf) {
        aos_free(arg);
        return;
    }
    ...
    const ip_addr_t *addr, u16_t port)
{
    ...
    if ((signed)udp_header.id < 0) {    // 发送完成，client端等待应答
        ...
        printf("%.4f(%.4f %.4f %.4f) Mbps, out of order: %lu.\r\n",
                f_now,
                ctx->f_min,
                f_avg,
                ctx->f_max,
...
static void iperf_server(void *arg)
{
    ...
    recv_data = (uint8_t *)aos_malloc(IPERF_BUFSZ);
    if (recv_data == NULL)
    {
        printf("No memory\r\n");
        goto __exit;
...
__exit:
    ...
    if (recv_data) aos_free(recv_data);
    if (arg) aos_free(arg);
    exit_flag = 0;
}
