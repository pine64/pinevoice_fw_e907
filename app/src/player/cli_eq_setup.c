...
static void cmd_peq_set_func(char *wbuf, int wbuf_len, int argc, char **argv)
{   
    int ret = -1;
        ...
        if (strcmp(argv[1], "GET") == 0) {
            ...
            for (int i = 0; i < (sizeof(g_eq_params)/sizeof(t_aefEqFilterParam)); i++) {
                ...
                if(g_eq_params[i].enable){
                    printf("MSP_PEQ GET %d ON %s %.2f %.2f %.1f\r\n", i, aefBQFType, g_eq_params[i].gain, g_eq_params[i].q, g_eq_params[i].freq);
                }else{
                    printf("MSP_PEQ GET %d OFF %s %.2f %.2f %.1f\r\n", i, aefBQFType, g_eq_params[i].gain, g_eq_params[i].q, g_eq_params[i].freq);
                }
            }
        } else {
...
static void cmd_limiter_set_func(char *wbuf, int wbuf_len, int argc, char **argv)
{
    int ret = -1;
...
static void cmd_gain_set_func(char *wbuf, int wbuf_len, int argc, char **argv)
{
    int ret = -1;
...
static void cmd_compander_set_func(char *wbuf, int wbuf_len, int argc, char **argv)
{
    int ret = -1;
    ...
    if (argc == 2) {
        if (strcmp(argv[1], "GET") == 0) {
            ...
            printf("MSP_COMPAND GET %s %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f %.4f\r\n", enable, g_companderParam.compress_trd, g_companderParam.expand_trd, g_companderParam.attack_time*1000,
                                                                                                g_companderParam.release_time*1000, g_companderParam.average_time*1000, g_companderParam.hold_time*1000,
                                                                                                g_companderParam.compr_ratio, g_companderParam.expand_ratio, g_companderParam.width);
        } else {
            printf("param error!\r\n");
        }
    } else if (argc == 12) {
        ...
        g_companderParam.compress_trd = atof(argv[3]);
        g_companderParam.expand_trd = atof(argv[4]);
        g_companderParam.attack_time = atof(argv[5])/1000;
        g_companderParam.release_time = atof(argv[6])/1000;
        g_companderParam.average_time = atof(argv[7])/1000;
        g_companderParam.hold_time = atof(argv[8])/1000;
        g_companderParam.compr_ratio = atof(argv[9]);
        g_companderParam.expand_ratio = atof(argv[10]);
